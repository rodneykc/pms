...
function getDocumentSigner($enc): string
{
    $sql = sqlStatement(
        "SELECT CONCAT(u.lname, ', ', u.fname) as name FROM `users` u " .
                    "JOIN `esign_signatures` es ON `es`.`uid` = `u`.`id` " .
                    "JOIN forms f ON `f`.`id` = `es`.`tid` " . 
                    "WHERE `es`.`tid` = `f`.`id` AND `f`.`encounter` = ? ", [$enc]    );
    $signers = '';
    $c = 0;
...
<thead class='thead-light'>
  ...
  <th>
        ...
        <?php
            //if($form_esigned) {
                echo "<th>" . $form_esigned. xlt(' Signed by') . "</th>";
            //}
        ?>
