...
<head>
 ...
 occurrence of one of these set days. */
  ...
  if (!empty($_POST['form_action']) && ($_POST['form_action'] == "save")) {
    ...
    if ($eid) {
      ...
      if ($GLOBALS['select_multi_providers'] && $row['pc_multiple']) {
        ...
        if ($_POST['recurr_affect'] == 'current') {
          ...
          // Vinix Code - Webhook
          $args = [
            "category" => $_POST['form_title'],
            "title" => $_POST['form_patient'],
            "facility" => $_POST['facility'],
            "event_date" => $event_date,
            "start_date" => $starttime,
            "form_apptstatus" => $_POST['form_apptstatus'],
            "end_date" => $endtime,
            "appt_id_provider" => (int) $_POST["form_provider"],
            "form_pid" => (int) $_POST["form_pid"],
            "form_repeat" => $_POST["form_repeat"],
            "form_enddate" => $_POST["form_enddate"],
            "form_comments" => $_POST["form_comments"],
            "multi_event" => $my_recurrtype,
          ];

          webhook($eid, $args, 'update');
          // Vinix Code - Webhook
        }
      }
    ...
    // Vinix Code - Webhook
    webhook($eid, $_POST["form_provider"], 'delete');
    // Vinix Code - Webhook 
  }
